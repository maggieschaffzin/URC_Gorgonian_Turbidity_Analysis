...
ggplot(combined_data2, aes(x = SiteType, y = mean_density, fill = ifelse(SiteType == "NR", DepthZone, SiteType))) +
  geom_boxplot(position = position_dodge(width = 0.9), width = 0.5, alpha = 0.7, color = "black") +
  scale_fill_manual(values = c("Inner" = pastel_colors[1], "Middle" = pastel_colors[2], "Outer" = pastel_colors[3], "Deep" = pastel_colors[5], "AR" = pastel_colors[4])) +
  geom_text_repel(aes(label = ifelse(SiteType == "NR" & mean_density >= 54, Site, "")), size = 3, box.padding = 0.3) +
  labs(
    x = "Site Type",
    y = "Mean Gorgonian Density (100m²)",
...
p9 <- ggplot(data = habitat_info_density_3, aes(x = Substrate_SD, y = mean_density)) +
  ...
  geom_line(data = combined_data2, aes(x = Substrate_SD, y = predicted, group = Region), color = "black") +
  geom_ribbon(data = combined_data2, aes(x = Substrate_SD, ymin = predicted - se, ymax = predicted + se, fill = Region), alpha = 0.1) +
  labs(x = "Substrate SD", y = "Mean Gorgonian Density (per 100m²)") +  
