...
crucore<- read_csv('UPC_crustose_coralline.r.csv')
...
#Creating a subset of the macrodensity dataset
macrodensity_subset <- macrodensity %>%
  dplyr::select(Site, DepthZone, SpeciesGroupF, Density_m2)
...
# Filter to only include rows where Species is "Muricea californica"
dat_gorg_filtered <- dat_gorg %>%
  filter(Species == "Muricea californica")

# Join the filtered data with min_euclidean_density_filter
min_euclidean_density_final <- left_join(min_euclidean_density_filter, dat_gorg_filtered, by = "Site")

#Grouping data by depth zone, species, and site or analysis
dat_gorg <- gorgoniandensity_clean %>%
  group_by(DepthZone, Site) %>%
  summarise(Mean_Density_100m2 = 100*mean(Density_m2))

# Habitat Dataset
habitat_info <- read_csv('habitat_characteristics.csv')

# Filter mean_density_per_site to only include sites present in habitat_info
mean_density_per_site_clean <- mean_density_per_site %>%
  filter(Site %in% habitat_info$Site)
#Checking length
length(unique(mean_density_per_site_clean$Site))

#Merging Datasets
habitat_info_density <- left_join(mean_density_per_site_clean, habitat_info, by = "Site")

# Inspect the first few rows of the merged dataset
head(habitat_info_density)

# Filter mean_density_per_site to only include sites present in habitat_info
sitedata_clean <- sitedata %>%
  filter(Site %in% habitat_info$Site)
#Checking length
length(unique(sitedata_clean$Site))

#Joining datasets
habitat_info_density_2 <- left_join(habitat_info_density, sitedata_clean, by = "Site")

#CCA Dataset
crucoregorg<- left_join(mean_density_per_site_clean, crucore, by = "Site")

#CCA dataset including Region
crucoregorg_region<- left_join (crucoregorg, sitedata_clean, by = "Site")
